...
fn build(sysroot: Option<&str>) -> io::Result<()> {
    ...
    // zlib (macOS SDK ships zlib; the upstream build-zlib feature only handles Linux)
    if env::var("CARGO_FEATURE_BUILD_ZLIB").is_ok() && matches!(target_os.as_str(), "macos") {
        configure.arg("--enable-zlib");
    }
